...
from PyQt5.QtWidgets import QMainWindow, QAction, qApp, QApplication, \
    QLabel, QTableView, QDialog, QPushButton, \
...
# GUI.
def gui_make_model(database):
    list_users = database.active_users_list()
    list = QStandardItemModel()
    list.setHorizontalHeaderLabels(['Username', 'IP', 'Port', 'TimeCon'])
...
def make_stat_model(database):    
    history_list = database.message_history()

    # Объект модели данных:
    list = QStandardItemModel()
    list.setHorizontalHeaderLabels(
        ['username', 'last time', 'msg sends',
         'msg income'])
